...
func (c *ClientPostgres) CreateClient(projectId int, client cargodelivery.Client) (int, error) {
	var id int
	createClientQuery := fmt.Sprintf("INSERT INTO %s (project_id, name, address) VALUES ($1, $2, $3) RETURNING id", clientsTable)
	row := c.db.QueryRow(createClientQuery, projectId, client.Name, client.Address)
...
func (c *ClientPostgres) GetAllClients(projectId int) ([]cargodelivery.Client, error) {
	...
	query := fmt.Sprintf("SELECT id, name, address FROM %s WHERE project_id = $1", clientsTable)
...
func (c *ClientPostgres) GetClientById(projectId, clientId int) (cargodelivery.Client, error) {
	...
	query := fmt.Sprintf("SELECT id, name, address FROM %s WHERE project_id = $1 AND id = $2", clientsTable)

...
const ListRoutesItem = ({ item, index }: ListRoutesItemProps) => {
    const { deleteRoute, selectRouteIndex, setSelectRouteIndex } = useContext(RoutesContext)
    const date = useMemo(() => {
    ...
    return (
        ...
        >
            ...
            <div className='flex justify-between mt-1'>
                <button className='bg-slate-500 rounded hover:bg-slate-400 px-2'>Подробно</button>
                <DelBtn onClick={() => deleteRoute(item.id)} />
            </div>
        </div>
    );
}

...
function SideBar() {
    const { currentProjectId, projects } = useContext(ProjectsContext)
    const { logout } = useContext(AuthContext)

    return (
        <aside className="flex w-60 h-full flex-col bg-pink-500">
            <header className="flex h-14 min-h-[56px] items-center justify-center bg-slate-800">
                <h1 className="text-xl text-white">Cargo Delivery</h1>
            </header>
            <main className="h-[calc(100%_-_112px)] w-full bg-slate-700">
                <div className="flex h-[56px] text-slate-300 items-center justify-between p-2 shadow-xl">
                    <span>Проекты</span>
                    <InputProjectName id={currentProjectId} btnContent={<p className="mx-3 my-1.5">+</p>} mode='add' />
                </div>
                <div className="h-[calc(100%_-_56px)] w-screen overflow-y-auto overflow-x-hidden">
                    {!projects || !projects.length ?
                        <div className="text-slate-300 w-60 text-center p-5">Нет проектов</div>
                        :
            ...
            <footer className="bg-slate-800 h-14 p-3 min-h-[56px] items-center">
